...
![function function, what's your function?](../img/shr_function.jpg)
...
my_triangles %>% 
  mutate(tri_a = tri_area(tri_h, tri_b, rnd = 0))
...
* the area of a triangle where the height and base are integers will always be either exactly an integer or exactly 0.5 remainder ... so rounding to more than one decimal doesn't produce any greater precision.

* the `round()` function in R automatically rounds an exact 0.5 to the nearest _even_ number. This is noted in the documentation, and is in conformance with international standards:
...
* on the ISO website [ISO/IEC/IEEE 60559:2011](https://www.iso.org/standard/57469.html)

* [The IEEE Standard for Floating-Point Arithmetic (IEEE 754)](https://en.wikipedia.org/wiki/IEEE_754) (Wikipedia)

* [Round half to even](https://en.wikipedia.org/wiki/Rounding#Round_half_to_even) (Wikipedia); "A tie-breaking rule without positive/negative bias _and_ without bias toward/away from zero is **round half to even**."
