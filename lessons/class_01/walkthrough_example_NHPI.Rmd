...
![_The data science process_](data-science.png)
...
Our end point will be this chart^[The original source code, created by Martin Monkman with contributions by Julie Hawkins and Stephanie Yurchak, can be found here:  https://github.com/bcgov/CANSIM-dataviewer/blob/master/scr/NHPI_cansim.Rmd]
...
```{r setup-walkthrough, message=FALSE, warning=FALSE}
# tidyverse
library(tidyverse)
...
The `max()` function returns the biggest value in a series. Below, it is used to return the largest value in the "REF_DATE" variable; in other words, the most recent month that has been published. (Remember that when the chart at the beginning of this chapter was created, )

```{r}

max(thedata$REF_DATE)
