...
Strings are characters, numbers, etc. that are enclosed inside quotation marks.
...
The {stringr} function `str_view()` shows where in our string the pattern appears. First we create an object "x" with three strings in it, then find two different patterns.
...
### 5c. Selecting by string  {dplyr::select} -
