...
* note that `ref_date` is a date type, so the `scale_x_` function is `scale_x_date`
  ...
  - this also allows us to specify the breaks and labels using `year` -- if you had one year of data, you could use `month` (note: you need to have the package {lubridate} loaded via the `library()` function)
