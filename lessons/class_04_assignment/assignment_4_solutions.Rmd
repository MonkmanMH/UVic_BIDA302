...
>
> * There is one variable that is common to all three tables (the key), but in only one of the tables will there be only one instance of each value (where it is unique). 
>
> * Remember that the `Batting_2016` and `Salaries_2016` are extracts from longer tables that cover multiple seasons.
> 
...
# note: this is an anti_join:
#  - _drops_ all of the observations in x that have a match in y
# a semi-join _keeps_ all of the observations in x that have a match in y
