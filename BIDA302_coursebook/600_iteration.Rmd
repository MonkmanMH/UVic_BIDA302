...
A simple example, with 4 columns of 50 records each, with a normal distribution centred on 100. (This is a variation on what's in the "Iteration" chapter of _R4DS_.)
...
Or rather than repeat the same function three times, you can write a for loop:

```{r}
# define the output object before you start the loop --
#   this makes your loop more efficient
# notice that it uses `ncol()` to count the number of columns in our dataframe "df"
...
Note the use of a regular expression "\\.xls*" to define the pattern for any Excel file, including the wild card "*" which identifes both the older ".xls" extension or the newer ".xlsx".

```{r}

files <- dir("data_monthly/", pattern = "\\.xls*", full.names = TRUE)
files
