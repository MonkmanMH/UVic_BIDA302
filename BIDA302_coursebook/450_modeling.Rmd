...
* Demonstrate an understanding of the input and output components of a regression model 
...
In this example, the trend is fairly clear: as the value of "x" increases, so does "y". 
...
The line minimizes the distance between all of the points (more precisely, it's the minimum _squared_ distance). If we were to change the angle of the line in any way, it would increase the total distance. The plot below shows the distance between the measured points and the prediction line. (Note that there is some overlap.) 
...
If `x` = 2.5, then `y` = 4.124 + (2.052 * 2.5), which is 9.254. Look back at the line of best fit--if `x` = 2.5, the `y` point on the line is just below 10. 
...
Let's start with just the youtube and see how well it predicts sales. First, we will plot the relationshp:
...
The way to read this is that we will get 8.44 units sold with zero youtube advertising, but with every dollar of youtube spending we would expect an increase of 0.48 in sales. So if we spent $1,000, we would see a total of 56.4 units sold (`sales = 8.44 + 0.048 * 1000`) 
...
(Note that the data set `sim1` comes from the package {modelr}.)
...
Another version of the syntax that skips the creation of an object.
