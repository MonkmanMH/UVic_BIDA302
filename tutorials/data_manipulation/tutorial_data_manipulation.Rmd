...
Similarly to **`readr`**, **`dplyr`** and **`tidyr`** are also part of the tidyverse. These packages were loaded in R's memory when we call `library(tidyverse)`.
...
Take a look at the `interviews` data frame:
...
```{r, pipes_1_example, exercise=TRUE, exercise.eval=TRUE}
...
```{r, pipes_2_example, exercise=TRUE, exercise.eval=TRUE}
...
```{r, pipes_3_example, exercise=TRUE, exercise.eval=TRUE}
interviews %>%
    filter(village == "God") %>%
...
```{r, pipes_assignment_example, exercise=TRUE, exercise.eval=TRUE}
