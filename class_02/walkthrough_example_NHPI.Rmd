...
select our variables of interest to make the dataframe more manageable, add month-over-month and year-over-year values
...
thedata_BC_Can %>%
  ...
  select(year, month, value, 
         "from previous month" = mom_pct, 
         "from same month, previous year" = yoy_pct) %>%
...
tbl_month_BC %>%
  kable(caption = "NHPI, British Columbia, Total (house & land)", digits = 1) %>%
  kable_styling(bootstrap_options = "striped") %>%
  row_spec(0, bold = T, font_size = 14) %>%
  row_spec(1, bold = T) %>%
  add_header_above(c(" " = 3, "percentage point change" = 2), font_size = 14)
