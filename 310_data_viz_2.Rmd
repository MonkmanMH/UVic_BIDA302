...
ggplot(penguin_species_bill, aes(x = species, y = bill_length_mean)) +
    geom_col()
...
ggplot(penguin_species_bill, aes(x = reorder(species, bill_length_mean), y = bill_length_mean)) +
    geom_col()

```

What happens if you were to add a minus sign (hyphen) in front of the `bill_length_mean` of the `reorder()` function? The full code would look like this:
